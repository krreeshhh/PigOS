...
if command -v yay > /dev/null; then
  print_warning "Skipping yay installation (already installed)."
	...
	run_command "bash -c 'cd /tmp && git clone https://aur.archlinux.org/yay-git.git && cd yay-git && makepkg --noconfirm -si'" "Clone and build YAY (Must)/Breaks the script" "no" "no" 
fi
